...
def generate_uber_ride_confirmation(custom_pickup=None, custom_destination=None, custom_ride_type=None):
    # Generate timestamps
    ...
    dropoff_address = custom_destination if custom_destination else fake.address().replace('\n', ', ')
    ...
    vehicle_type = custom_ride_type if (custom_ride_type and custom_ride_type in VEHICLE_TYPES_LIST) else random.choice(VEHICLE_TYPES_LIST)
    vehicle_type_id = VEHICLE_TYPE_ID_MAP.get(vehicle_type, 1)
...
if EVENT_HUB_CONNECTION_STR and EVENT_HUB_NAME:
    try:
        ...
        print("Event Hub Producer initialized successfully.")
    except Exception as e:
        init_error = str(e)
        print(f"Error initializing Event Hub Producer: {init_error}")
else:
    print("Event Hub credentials missing in .env file.")
...
def send_event():
    if not producer:
        error_msg = init_error if init_error else "Event Hub producer is not initialized. Please check your .env file."
        return jsonify({
            "error": error_msg
        }), 500
    ...
    try:
        ...
        # Generate rich data based on user input
        rich_data = generate_uber_ride_confirmation(
            custom_pickup=data.get('pickup_location'),
            custom_destination=data.get('destination'),
            custom_ride_type=data.get('ride_type')
        )
        ...
        event_data_batch.add(EventData(json.dumps(rich_data)))

        # Send the batch of events to the event hub
        producer.send_batch(event_data_batch)
        
        return jsonify({
            "message": "Rich ride confirmation event sent successfully to Azure Event Hub",
            "data": rich_data
        }), 200
